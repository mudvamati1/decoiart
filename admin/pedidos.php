...
if (isset($_GET['limite'])  && isset($_GET['filtro']) && isset($_GET['orden'])) {
    ...
    $totalQuery = $conexion->query("SELECT COUNT(*) FROM ventas WHERE rut_vendedor = '" . $arregloUsuario['rut'] . "'") or die($conexion->error);
    $totalProductos = mysqli_fetch_row($totalQuery);
    $totalBotones = round($totalProductos[0] / $limite);
    $resultado = $conexion->query("SELECT v.*,pr.*, p.nombre, p.imagen, e.region, e.direccion, e.comuna, e.estado_envio, 
    u.rut,u.nombre,u.apellidos,u.correo,es.*, r.id, r.nombre, c.id, c.nombre, pr.id as 'id_orden'
     FROM ventas v 
    ...
    INNER JOIN usuarios u ON u.rut = v.rut_usuario
    ...
    INNER JOIN region r ON r.id = e.region
    INNER JOIN comuna c ON c.id = e.comuna
    WHERE v.rut_vendedor ='" .  $arregloUsuario['rut'] . "' ORDER BY v.id DESC  limit " . $_GET['limite'] . ", " . $limite . "") or die($conexion->error);
  } else {
    $limite = 10; //Productos por pagina
    $totalQuery = $conexion->query("SELECT COUNT(*) FROM ventas WHERE rut_vendedor = '" . $arregloUsuario['rut'] . "'") or die($conexion->error);
    $totalProductos = mysqli_fetch_row($totalQuery);
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="dropdown show">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                <a class="dropdown-item" href="./pedidos.php?filtro=pr.precio&orden=DESC">Precio de mayor a menor</a>
                                <a class="dropdown-item" href="./pedidos.php?filtro=pr.precio&orden=ASC">Precio de menor a mayor</a>
                                <a class="dropdown-item" href="./pedidos.php?filtro=v.id&orden=DESC">De mas reciente a mas antiguo</a>
                                <a class="dropdown-item" href="./pedidos.php?filtro=v.id&orden=ASC">De mas antiguo a mas reciente</a>
                                <a class="dropdown-item" href="./pedidos.php?filtro=pr.cantidad&orden=DESC">De mayor cantidad a menor cantidad</a>
                                <a class="dropdown-item" href="./pedidos.php?filtro=pr.cantidad&orden=ASC">De menor cantidad a mas cantidad</a>
                            </div>
                            ...
                            <div class="card">
                                ...
                                <div id="collapse<?php echo $fila[5] ?>" class="collapse " aria-labelledby="heading<?php echo $fila[5] ?> data-parent=" #accordion">
                                    <div class="card-body">
                                        ...
                                        <span class="d-block text-primary h6 text-uppercase"> Nombre Comprador:<?php echo $fila[21] . " " . $fila[22]; ?></span>
                                        <span class="d-block text-primary h6 text-uppercase">Correo Comprador:<?php echo $fila[23]; ?></span>
                                        ...
                                        <form action="cambiar-estado.php" method="POST">
                                            ...
                                            <select name="estatus" class="form-select">
                                                ...
                                                while ($f = mysqli_fetch_array($res)) { ?>
                                                    ...
                                                    <option value="<?= $f[0] ?>" <?= ($f[0] == $fila[12]) ? 'selected="selected"' : '' ?>>
                                                        <?= $f[1] ?>
                                                    </option>
                                        ...
                                        <span class="d-block text-primary h6 text-uppercase"><?php echo $fila[27]; ?></span>
                                        <span class="d-block text-primary h6 text-uppercase">Comuna: <?php echo $fila[29]; ?></span>
                                        <span class="d-block text-primary h6 text-uppercase">Direccion: <?php echo $fila[17]; ?></span>
                                        <form action="cambiar-envio.php" method="POST">
                                            ...
                                            <?php
                                                ...
                                                while ($f = mysqli_fetch_array($res)) { ?>
                                                    ...
                                                    <option value="<?= $f[0] ?>" <?= ($f[0] == $fila[19]) ? 'selected="selected"' : '' ?>>
                                                        <?= $f[1] ?>
                                                    </option>
          ...
          <?php
           ...
           if (isset($_GET['filtro']) && isset($_GET['orden'])) {
            if (isset($_GET['limite'])) {
              if ($_GET['limite'] > 0) {
                echo '<li class="list-group-item "><a href="pedidos.php?filtro=' . $_GET['filtro'] . '&orden=' . $_GET['orden'] . '&limite=' . ($_GET['limite'] - 10) . '">&lt;</a></li>';
              }
            }
            for ($k = 0; $k < $totalBotones; $k++) {
              echo '<li class="list-group-item "><a href="pedidos.php?filtro=' . $_GET['filtro'] . '&orden=' . $_GET['orden'] . '&limite=' . ($k * 10) . '">' . ($k + 1) . '</a></li>';
            }
            if (isset($_GET['limite'])) {
              ...
              echo '<li class="list-group-item "><a href="pedidos.php?filtro=' . $_GET['filtro'] . '&orden=' . $_GET['orden'] . '&limite=10">&gt;</a></li>';
            }

          } else {
            if (isset($_GET['limite'])) {
              if ($_GET['limite'] > 0) {
                echo '<li class="list-group-item "><a href="pedidos.php?limite=' . ($_GET['limite'] - 10) . '">&lt;</a></li>';
              }
            }
            for ($k = 0; $k < $totalBotones; $k++) {
              echo '<li class="list-group-item "><a href="pedidos.php?limite=' . ($k * 10) . '">' . ($k + 1) . '</a></li>';
            }
            if (isset($_GET['limite'])) {
              if ($_GET['limite'] + 10 < $totalBotones * 10) {
                echo ' <li class="list-group-item "><a href="pedidos.php?limite=' . ($_GET['limite'] + 10) . '">&gt;</a></li>';
              ...
              echo '<li class="list-group-item "><a href="pedidos.php?limite=10">&gt;</a></li>';
            }
          }
