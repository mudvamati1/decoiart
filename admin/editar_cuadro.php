...
if ($_FILES['imagen']['name'] != '') {
  ...
  if ($extension == 'jpg' || $extension == 'png') {
    if (move_uploaded_file($_FILES['imagen']['tmp_name'], $carpeta . $nombrefinal)) {
      $fila = $conexion->query("SELECT imagen FROM cuadros WHERE id=" . $id_producto);
      $id = mysqli_fetch_row($fila);
      if (file_exists('../Carrito/images/' . $id[0])) {
        unlink('../Carrito/images/' . $id[0]);
      }

      $conexion->query("UPDATE cuadros SET imagen ='" . $nombrefinal . "' WHERE id=" . $id_producto);
    }
  }
}


$conexion->query("UPDATE cuadros SET nombre ='" . $nombre . "',
