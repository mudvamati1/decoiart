...
if ((empty($rut)) || strlen($rut) < 9) {
    echo "<script>alert('Rut vacio o largo no valido ');window.location='vendedores.php'</script>";
    exit();
...
if (!preg_match("/^[0-9]*$/", $parteNumerica)) {
    echo "<script>alert('La parte numerica solo debe contener numeros ');window.location='vendedores.php'</script>";
    exit();
...
if (strlen($guionYVerificador) != 2) {
    echo "<script>alert('Error en el largo del digito verificador  ');window.location='vendedores.php'</script>";
    exit();
...
if (!preg_match('/(^[-]{1}+[0-9kK]).{0}$/', $guionYVerificador)) {
    echo "<script>alert('El digito verificador no es valido ');window.location='vendedores.php'</script>";
    exit();
...
if (!preg_match("/^[0-9.]+[-]?+[0-9kK]{1}/", $rut)) {
    echo "<script>alert('Error al digitar el rut ');window.location='vendedores.php'</script>";
    exit();
...
if ($dvr == strtoupper($dv)) {
    ...
    echo "<script>alert('El rut ingresado no es valido');window.location='vendedores.php'</script>";
    exit();
...
if (mysqli_num_rows($verficar_correo) > 0 || mysqli_num_rows($verficar_correo3) > 0) {
    ...
    echo "<script>alert('Correo Existente ');window.location='vendedores.php'</script>";
    exit();
}


$verficar_Rut = mysqli_query($conexion, "SELECT * FROM vendedores WHERE rut_vendedor='$rut' ");

if (mysqli_num_rows($verficar_Rut) > 0) {

    echo "<script>alert('Rut Existente');window.location='vendedores.php'</script>";
    exit();
}
    ...
    if ($rut && $nombre && $apellido && $email && $password && $direccion) {
        if (isset($_POST['btnsendd'])) {
            ...
            if ($fecha_actual < $fecha_nacimiento) {
                echo "<script>alert('La fecha no puede ser mayor a la actual');window.location='vendedores.php'</script>";
            } else {
                $queryregistrar = "INSERT INTO vendedores (rut_vendedor,nombre,apellidos,correo,password,fecha_nac,comuna,direccion,region,estado,created_at,nivel)
  VALUES ('$rut','$nombre','$apellido','$email','$password','$fechaDN',$comuna,'$direccion',$region,0,NOW(),1)";

                if (mysqli_query($conexion, $queryregistrar)) {
                    $ok = "Se ha agregado el vendedor satisfactoriamente";
                    $_SESSION['success'] = $ok;
