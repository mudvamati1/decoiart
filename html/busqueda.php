...
<body>
                ...
                <div class="row mb-5">
                    <div class="col-md-9 order-2">
                        ...
                        <div class="row mb-5">
                            ...
                            $totalQuery = $conexion->query("SELECT COUNT(*) FROM cuadros WHERE id_categoria=$botones_busqueda ") or die($conexion->error);
                            $totalProductos = mysqli_fetch_row($totalQuery);
                            $totalBotones = round($totalProductos[0] / $limite);

                            if (isset($_GET['limite'])) {

                                $resultado = $conexion->query("select cuadros.*, categorias.nombre as categoria from cuadros 
                INNER JOIN categorias ON cuadros.id_categoria = categorias.id
                WHERE 
                 cuadros.nombre LIKE '%" . $_GET['texto'] . "%' or 
                 cuadros.descripcion LIKE '%" . $_GET['texto'] . "%' or
                 cuadros.medidas LIKE '%" . $_GET['texto'] . "%' or
                 categorias.nombre LIKE '%" . $_GET['texto'] . "%' 
                
                 order by id ASC  limit " . $_GET['limite'] . ", " . $limite) or die($conexion->error);
                            } else {
                                $resultado = $conexion->query("select cuadros.*, categorias.nombre as categoria from cuadros 
                INNER JOIN categorias ON cuadros.id_categoria = categorias.id
                WHERE 
    ...
    <script src="../Carrito//jquery-ui.js"></script>
